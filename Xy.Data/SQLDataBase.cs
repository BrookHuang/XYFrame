...
namespace Xy.Data {
    public class SQLDataBase : IDataBase {
        ...
        public class TransException:Exception{
            public TransException(string text):base(text){
            }
        }
        ...
        public int InvokeProcedureResult(Procedure inProcedure) {
            SqlCommand _command;
            if (string.IsNullOrEmpty(inProcedure.Command)) {
                _command = new SqlCommand(inProcedure.Name, _con);
                _command.CommandType = CommandType.StoredProcedure;
            ...
            if(!inProcedure.HasReturnParameter)
                _command.Parameters.AddWithValue("xy_Procedure_Return", _resultInt).Direction = ParameterDirection.ReturnValue;
            this.Open();
            try {
                if (_trans != null) _command.Transaction = _trans;
            ...
            if (inProcedure.HasReturnParameter) {
                _resultInt = (int?)getOutputParameters(_command, inProcedure);
            } else {
                _resultInt = (int?)_command.Parameters["xy_Procedure_Return"].Value;
            }
            if (_resultInt != null) {
                return Convert.ToInt32(_resultInt);
            } else {
